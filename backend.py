...
class TravelState(TypedDict, total=False):
    ...
    trip_constraints: dict[str, Any]# exacting multiple entity
...
# Shared helpers
# =========================
#These are the agents available in my application
KNOWN_AGENTS = {
    "flight_agent",
    "hotel_agent",
    "weather_agent",
...
# where i need to call LLM so i create separate function
...
# what are LLM return it convert JSON format is easy communication between agents 
...
"""
    ...
    # Fail open on parser/model errors so a temporary JSON-format issue does not.JSON makes it easier for your Python code to process the LLM's decision.
    # break the original travel-planning behavior.
    #If the guardrail itself has a temporary problem, don't block the user's travel request.
    try:
...
# =========================
# Dynamic Supervisor 
# Routing Supervisor = decides WHAT agents are needed.
